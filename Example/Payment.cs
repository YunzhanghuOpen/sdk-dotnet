...
namespace Aop.Api.Example
{
    ...
    public class Payment
    {
        ...
        public static void CreateBatchOrder()
        {
            ...
            CreateBatchOrderRequestModel model = new CreateBatchOrderRequestModel
            {
                ...
                OrderList = new BatchOrderInfo[]
                {
                    ...
                    RealName = "李四",
                    IDCard = "440524188001010014",
                    CardNo = "6228888888888888887",
                    PhoneNo = "188****8888",
                    Pay = "2.00",
                    PayRemark = "测试备注",
                    NotifyURL = "https://www.example.com",
        ...
        /// 查询劳务模式单笔订单信息
        /// </summary>
        public static void GetOrderLxlw()
        {
            // 实例化具体 API 对应的 request 类
            GetOrderLxlwRequest request = new GetOrderLxlwRequest();

            // 配置请求参数
            GetOrderLxlwRequestModel model = new GetOrderLxlwRequestModel
            {
                OrderID = "180490818101749",
                Channel = "银行卡",
                DataType = ""
            };
            request.SetBizModel(model);
            ...
            try
            {
                GetOrderLxlwResponse res = Client.Execute(request);
                Console.WriteLine(res.Body);
                if (res.IsSuccess)
                {
                    // 操作成功
                    GetOrderLxlwResponseModel data = res.Data;
                }
                else
                {
                    // 失败返回
        ...
        /// 查询劳动者退款订单信息
        /// </summary>
        public static void GetLaborRefundOrder()
        {
            // 实例化具体 API 对应的 request 类
            GetLaborRefundOrderRequest request = new GetLaborRefundOrderRequest();

            // 配置请求参数
            GetLaborRefundOrderRequestModel model = new GetLaborRefundOrderRequestModel
            {
                DealerID = Config.DealerID,
                OrderID = "202009010016562012987",
                Channel = "银行卡"
            ...
            try
            {
                GetLaborRefundOrderResponse res = Client.Execute(request);
                Console.WriteLine(res.Body);
                if (res.IsSuccess)
                {
                    // 操作成功
                    GetLaborRefundOrderResponseModel data = res.Data;
                }
                else
                {
                    // 失败返回
        ...
        /// 取消批次中单笔订单
        /// </summary>
        public static void CancelOrderInBatch()
        {
            // 实例化具体 API 对应的 request 类
            CancelOrderInBatchRequest request = new CancelOrderInBatchRequest();

            // 配置请求参数
            CancelOrderInBatchRequestModel model = new CancelOrderInBatchRequestModel
            {
                DealerID = Config.DealerID,
                BrokerID = Config.BrokerID,
            ...
            try
            {
                CancelOrderInBatchResponse res = Client.Execute(request);
                Console.WriteLine(res.Body);
                if (res.IsSuccess)
                {
                    // 操作成功
                    CancelOrderInBatchResponseModel data = res.Data;
                }
                else
                {
                    // 失败返回
