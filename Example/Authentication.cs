...
namespace Aop.Api.Example
{
    ...
    public class Authentication
    {
        ...
        /// 非居民身份证验证名单审核结果查询
        /// </summary>
        public static void GetUserWhiteApproveInfo()
        {
            // 实例化具体 API 对应的 request 类
            GetUserWhiteApproveInfoRequest request = new GetUserWhiteApproveInfoRequest();

            // 配置请求参数
            GetUserWhiteApproveInfoRequestModel model = new GetUserWhiteApproveInfoRequestModel
            {
                RealName = "张三",
                IDCard = "EA3456789",
                CardType = "passport",
            ...
            try
            {
                GetUserWhiteApproveInfoResponse res = Client.Execute(request);
                Console.WriteLine(res.Body);
                if (res.IsSuccess)
                {
                    // 操作成功
                    GetUserWhiteApproveInfoResponseModel data = res.Data;
                }
                else
                {
                    // 失败返回
